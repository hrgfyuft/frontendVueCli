...
import { defineComponent, getCurrentInstance, ref, computed, onMounted, onUpdated, resolveDynamicComponent, h, nextTick } from 'vue';
...
const DefaultGridProps = {
    ...
    height: Object.assign(Object.assign({}, DefaultListProps.height), { validator: (val) => isNumber(val) }),
    ...
    width: Object.assign(Object.assign({}, DefaultListProps.width), { validator: (val) => {
            return isNumber(val);
        } }),
    ...
    return defineComponent({
        ...
        setup(props, { emit, expose }) {
            ...
            const windowStyle = computed(() => ([
                ...
                Object.assign({ direction: props.direction, height: isNumber(props.height) ? `${props.height}px` : props.height, width: isNumber(props.width) ? `${props.width}px` : props.width }, props.style),
            ]));
            ...
            const scrollVertically = (e) => {
                ...
                states.value = Object.assign(Object.assign({}, _states), { isScrolling: true, scrollDir: getScrollDir(_states.scrollOffset, scrollOffset), scrollOffset, updateRequested: false });
            ...
            const scrollTo = (offset) => {
                ...
                states.value = Object.assign(Object.assign({}, $(states)), { scrollOffset: offset, scrollDir: getScrollDir($(states).scrollOffset, offset), updateRequested: true });
                nextTick(resetIsScrolling);
            };
        ...
        render(ctx) {
            var _a;
            const { $slots, className, containerElement, data, getItemStyle, innerElement, itemsToRender, innerStyle, total, onScroll, states, useIsScrolling, windowStyle, } = ctx;
            ...
            if (total > 0) {
                for (let i = start; i <= end; i++) {
                    children.push((_a = $slots.default) === null || _a === void 0 ? void 0 : _a.call($slots, {
                        data,
                        key: i,
                        index: i,
    ...
    return defineComponent({
        ...
        setup(props, { emit, expose }) {
            ...
            const columnsToRender = computed(() => {
                ...
                const cacheBackward = !isScrolling || xAxisScrollDir === BACKWARD
                    ? Math.max(1, columnCache)
                    : 1;
                const cacheForward = !isScrolling || xAxisScrollDir === FORWARD
                    ? Math.max(1, columnCache)
                    : 1;
                return [
                    Math.max(0, startIndex - cacheBackward),
            ...
            const rowsToRender = computed(() => {
                ...
                const cacheBackward = !isScrolling || yAxisScrollDir === BACKWARD
                    ? Math.max(1, rowCache)
                    : 1;
                const cacheForward = !isScrolling || yAxisScrollDir === FORWARD
                    ? Math.max(1, rowCache)
                    : 1;
                return [
                    Math.max(0, startIndex - cacheBackward),
            ...
            const onScroll = (e) => {
                ...
                states.value = Object.assign(Object.assign({}, _states), { isScrolling: true, scrollLeft: _scrollLeft, scrollTop: Math.max(0, Math.min(scrollTop, scrollHeight - clientHeight)), updateRequested: false, xAxisScrollDir: getScrollDir(_states.scrollLeft, _scrollLeft), yAxisScrollDir: getScrollDir(_states.scrollTop, scrollTop) });
                nextTick(resetIsScrolling);
                emitEvents();
            };
            const getItemStyleCache = memo((_, __, ___) => ({}));
            const scrollTo = ({ scrollLeft, scrollTop, }) => {
                ...
                states.value = Object.assign(Object.assign({}, _states), { xAxisScrollDir: getScrollDir(_states.scrollLeft, scrollLeft), yAxisScrollDir: getScrollDir(_states.scrollTop, scrollTop), scrollLeft,
                    scrollTop, updateRequested: true });
                nextTick(resetIsScrolling);
            };
        ...
        render(ctx) {
            var _a;
            const { $slots, className, containerElement, columnsToRender, data, getItemStyle, innerElement, innerStyle, rowsToRender, onScroll, states, useIsScrolling, windowStyle, totalColumn, totalRow, } = ctx;
